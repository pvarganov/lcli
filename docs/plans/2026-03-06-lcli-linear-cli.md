...
- [ ] Создать `internal/config/config.go` для чтения токена из:
  1. Флаг `--token`
  2. Переменная окружения `LINEAR_API_KEY`
  3. Файл `~/.config/lcli/config.yaml`
- [ ] Команда `lcli auth login` — сохраняет токен в конфиг-файл
- [ ] Команда `lcli auth status` — показывает текущий токен (замаскированный)
- [ ] Написать тесты для чтения конфига из env и файла
